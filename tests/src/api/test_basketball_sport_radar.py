...
MOCK_NBA_TEAMS_IN_LEAGUE_RETURN = {
    'conferences': [
        {
            'divisions': [
                {
...
MOCK_WNBA_TEAMS_IN_LEAGUE_RETURN = {
    'conferences': [
        {
            'teams': [
                {
...
def mock_make_request(url: str):
    if 'league/hierarchy.json' in url and NBA_BASE_URL in url:
        return MOCK_NBA_TEAMS_IN_LEAGUE_RETURN

    if 'league/hierarchy.json' in url and WNBA_BASE_URL in url:
        return MOCK_WNBA_TEAMS_IN_LEAGUE_RETURN
...
class TestNbaSportRadar:
    ...
    @pytest.fixture(
        ...
        params=[
            Params(
                league_base_url=NBA_BASE_URL
            ),
            Params(
                league_base_url=WNBA_BASE_URL
            )
        ])
    def setup(self, request):
        ...
        return TestNbaSportRadar.Fixture(
            mock_api_client=mock_api_client,
            league_base_url=request.param.league_base_url,
            subject=BasketballSportRadar(api_client=mock_api_client, league_base_url=request.param.league_base_url)
        )
